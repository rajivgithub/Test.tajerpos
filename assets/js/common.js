...
$(document).on("change",".country-list",function() {
    ...
    if(country_name == "Saudi Arabia") {
        $(this).closest("form").find(".state-label").html(customerTranslations['province']+"<span>*</span>");
        $(this).closest("form").find(".area-input-field").removeClass('dnone');
    } else {
        $(this).closest("form").find(".state-label").html(customerTranslations['state']+"<span>*</span>");
        $(this).closest("form").find(".area-input-field").addClass('dnone');
    }
...
function validateAlphaSpecial(inputValue) {
    const trimmedValue = inputValue.val().trim();
...
function validatePattern(inputValue) {
    patternString = inputValue.data('pattern');
    trimmedValue = inputValue.val().trim();
...
function validateAlphaSpace(inputValue) {
    const trimmedValue = inputValue.val().trim();
    if (!/^[A-Za-z,. ]+$/.test(trimmedValue)) {
        return false;
    }
...
/*function validateFields(_this) {
    ...
    _this.closest("form").find(".required-field").each(function() {
        ...
        input_pattern = $(this).data("pattern");
        ...
        if((field_value == "") || (field_type == "radio" && $("#"+field_id+":checked").length == 0)) {
        ...
        if(input_pattern == "alphaSpecial" && field_value && validateAlphaSpecial($(this)) == false) {
            $(this).closest(".input-field-div").find(".error.error-message").text(langTranslations.alpha_special.replace(':attribute', field_label)).css("color", "#F30000");
            error++;
        }
        if(input_pattern == "alphaSpace" && field_value && validateAlphaSpace($(this)) == false) {
            $(this).closest(".input-field-div").find(".error.error-message").text(langTranslations.alpha_space.replace(':attribute', field_label)).css("color", "#F30000");
            error++;
        }
        ...
        if(field_label == "URL" && field_value != "" && validURL(field_value) != false) 
            isURLExist($(this)); 
        if(field_value != "" && min_value != undefined && parseInt(min_value) > 0 && (parseInt(field_value.length) < parseInt(min_value))) {
            $(this).closest(".input-field-div").find(".error.error-message").text(langTranslations.min.string.replace(':attribute', field_label).replace(':min', min_value)).css("color", "#F30000");
            error++;
        }
        if(field_value != "" && max_value != undefined && parseInt(max_value) > 0 && (parseInt(field_value.length) > parseInt(max_value))) {
            if(field_data_type == "show-border-error") {
                $(this).css("border","2px solid #F30000");
            }
            $(this).closest(".input-field-div").find(".error.error-message").text(langTranslations.max.string.replace(':attribute', field_label).replace(':max', max_value)).css("color", "#F30000");
            error++;
        }
    });
    return error;
...
function validateFields(_this) {
    ...
    _this.closest("form").find(".form-input-field").each(function() {
        ...
        max_value = $(this).attr("data-max");
        input_pattern = $(this).data("pattern");
        var summernoteContent = plainTextContent = "" ;
        var reqErrorMessage = langTranslations.required.replace(':attribute', field_label);
        var invalidErrorMessage = langTranslations.regex.replace(':attribute', field_label);
        if(((field_value == "") || (field_type == "radio" && $("#"+field_id+":checked").length == 0)) && $(this).hasClass('required-field')) {
            if($(this).attr("data-page") == "login")
                $(this).closest(".input-field-div").find(".error.error-message").text(reqErrorMessage).css("color",'rgb(239 134 94)');
            else if(field_data_type == "show-border-error")
                $(this).css("border","2px solid #F30000");
        ...
        if(field_data_type == "codeeditor") {
            var summernoteContent = $('#summernote').summernote('code');
            var decodedContent = he.decode(summernoteContent);
            var sanitizedContent = DOMPurify.sanitize(decodedContent, {
                ALLOWED_TAGS: ['b', 'i', 'u', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'p', 'a', 'img', 'br', 'hr', 'code', 'pre','table','thead','tbody','tr','th','td','iframe'],
            });
            $('#summernote').summernote('code', sanitizedContent);
        }
        if((field_value != "") && (((field_label == "Email Address" || field_type == 'email' || ((field_label == "User Name" || field_label == "Email"))) && IsEmail(field_value) == false) || (field_data_type == "image" && imageExtension($(this)) == false) || (field_label == "Amount" && isAmount(field_value) == false))) {
            if($(this).attr("data-page") == "login")
                $(this).closest(".input-field-div").find(".error.error-message").text(invalidErrorMessage).css("color",'rgb(239 134 94)');
            else
                $(this).closest(".input-field-div").find(".error.error-message").text(invalidErrorMessage).css("color", "#F30000");
        ...
        if(field_value != "" && input_pattern != undefined && input_pattern != "" && validatePattern($(this)) == false) {
            invalid_error_msg = ($(this).data("error-msg") != undefined) ? $(this).data("error-msg") : '';
            $(this).closest(".input-field-div").find(".error.error-message").text(invalid_error_msg).css("color", "#F30000");
            error++;
        }
        if(input_pattern == "alphaSpecial" && field_value && validateAlphaSpecial($(this)) == false) {
            $(this).closest(".input-field-div").find(".error.error-message").text(langTranslations.alpha_special.replace(':attribute', field_label)).css("color", "#F30000");
        ...
        if(field_label == "Email Address" && field_value != "") 
            isEmailExist(field_value,$(this),$(this).attr("data-type"));
        if(field_label == "URL" && field_value != "" && validatePattern($(this)) != false) 
            isURLExist($(this)); 
        if(field_value != "" && min_value != undefined && parseInt(min_value) > 0 && (parseInt(field_value.length) < parseInt(min_value))) {
            $(this).closest(".input-field-div").find(".error.error-message").text(langTranslations.min.string.replace(':attribute', field_label).replace(':min', min_value)).css("color", "#F30000");
            error++;
...
function checkUniqueField(selector, uniqueDataList ) {
    errorVariable = 0;
    var uniqueDataList = [];
    $(selector).each(function () {
        var fieldValue = $(this).val().trim();
        if (fieldValue !== "") {
            if (uniqueDataList.indexOf(fieldValue) !== -1) {
                $(this).css("border", "2px solid #F30000");
                errorVariable++;
            } else {
                $(this).css("border", "1px solid #86a4c3");
                uniqueDataList.push(fieldValue);
            }
        }
    });
